...
{
    public class TWSApiWrapper : EWrapper
    {
        
        Task connectionThread = null, msgReaderThread = null;
        ...
        public event EventHandler NoReady;

        public bool Ready { get; private set; }

        public void Run(string apiHost)
        {
            if (!Connect(apiHost))
                return;

            var token = connectionCancelToken.Token;
            var reader = new EReader(clientSocket, Signal);
            reader.Start();

            connectionThread = Task.Run( async () =>
            {
                while (!token.IsCancellationRequested)
                {
                    ...
                    {
                        if (NextOrderId > 0 && !Ready) 
                        {
                            PrepareAPI();
                            Ready = true;
                            OnReady?.Invoke(this, EventArgs.Empty);
                            //WebSocketServer.IBCommandReceived += WebSocketServer_IBCommandReceived;
                            //Console.WriteLine($"IB API is ready. Next order ID is {NextOrderId}");
                        }
            ...
            msgReaderThread = Task.Run( async () =>
            {
                while (!token.IsCancellationRequested)
                {
                    if (clientSocket.IsConnected())
        ...
        public void OrderPlace(string ticker, string action, int quantity, decimal price, string exchange)
        {
        ...
        {
            clientSocket.cancelOrder(nextOrderId - 1);
        }

        public void CancelOrdersAll()
        {
        ...
        {
            OrderStatuses[orderId].orderState.Status = status;
        ...
        public Dictionary<int, (Contract contract, Order order, OrderState orderState)> OrderStatuses { get; } = new Dictionary<int, (Contract contract, Order order, OrderState orderState)>();

        //! [openorder]
        public virtual void openOrder(int orderId, Contract contract, Order order, OrderState orderState)
        {
            OrderStatuses[orderId] = (contract, order, orderState);
        ...
        bool Connect(string host)
        {
            try
            {
                ...
                ClientSocket.eConnect(parts[0], int.Parse(parts[1]), 0);
            }
            catch
            {
                Console.WriteLine($"!!!!!!!!!!!!Invalid TWS api host '{host}'");
                NoReady?.Invoke(this, EventArgs.Empty);
                return false;
            }
            return true;
        }
        ...
        {
            if (new[] { 2103, 2105, 2157 }.Contains(errorCode))
                return;
        ...
        {
            //OnPrintReceived?.Invoke(this, (reqId, ticks, done));
        }
        //! [historicalticksbidask]
        public event EventHandler<(int, HistoricalTickBidAsk[], bool)> OnPrintReceived;
        //public event EventHandler<(int, HistoricalTickLast[], bool)> OnPrintReceived;
